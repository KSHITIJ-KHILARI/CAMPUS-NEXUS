...
async def lifespan(application: FastAPI):
    ...
    if redis_ok:
        ...
        logger.warning("Redis is offline. Operating with graceful in-memory fallback in development mode.")

    # Security validation on startup
    if settings.is_production:
        insecure_keys = {"dev-secret-key", "dev-secret-key-change-in-production", "change-this-to-a-long-random-secret-key-minimum-32-characters"}
        if settings.SECRET_KEY in insecure_keys or len(settings.SECRET_KEY) < 32:
            logger.critical("SECURITY ALERT: Running in PRODUCTION with insecure or short SECRET_KEY. Please set a strong SECRET_KEY in environment variables.")
...
def create_application() -> FastAPI:
    ...
    async def health_check(db: AsyncSession = Depends(get_session)):
        ...
        redis_status = "connected" if await is_redis_online() else "unavailable"
    ...
    @application.get("/api/v1/health/auth", tags=["health"])
    async def health_auth_alias(db: AsyncSession = Depends(get_session)):
